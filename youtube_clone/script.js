...
//             "x-rapidapi-key":"0ff471dfbamshef7b9f29fce5efdp13fa1fjsne033c0b89284",
...
const showUI=((list)=>{
    ...
    list.forEach((obj,idx)=>{
        ...
        let url=obj.videoThumbnails[0].url
...
const showSearchResult=((list)=>{
    ...
    list.forEach((obj,idx)=>{
        const newCard=document.createElement('div')
        newCard.className='card'
        let url=obj.video.thumbnails[0].url
        newCard.innerHTML=`
        
        <img src="${url}" width="100%" >
        <h5>${obj.video.author.title}</h5>
        <h4>${obj.video.title}</h4>
        
        `
        
        newCard.addEventListener("click",()=>{
            
            window.open(`./video.html?id=${obj.video.videoId}`,"_top")
        });
        root.appendChild(newCard)
    })
})
const handleSearch=(val)=>{
    const resp=fetch(`https://youtube138.p.rapidapi.com/search/?q=${val}&hl=en&gl=US`,{
        method:"GET",
        headers:{
            "x-rapidapi-host":"youtube138.p.rapidapi.com",
            "x-rapidapi-key":"0ff471dfbamshef7b9f29fce5efdp13fa1fjsne033c0b89284",
    ...
    resp.then((res)=>{
        ...
        pr2.then((data)=>{
            ...
            showSearchResult(data.contents)
        })
    }).catch((err)=>{
        alert(err.message)
    }
